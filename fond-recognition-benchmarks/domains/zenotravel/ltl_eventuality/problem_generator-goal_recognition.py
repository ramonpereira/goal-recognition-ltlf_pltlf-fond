...
def main() :
	...
	for problemNumber in range(1, int(numberOfProblems)+1):
		...
		for hypNumber in range(1, int(hypothesis)+1):
			...
			cmd = 'tar jcvf ' + domainName + '_ltl_' + typeGoal + '_p0' + str(problemNumber) + '_hyp-' + str(hypNumber) + '_full.tar.bz2' + ' domain.pddl initial_state.pddl hyps.dat obs.dat observability.dat real_hyp.dat'
			os.system(cmd)
