		...
		for hypNumber in range(1, int(hypothesis)+1):
			...
			for obs in observability:
				for alternativeObs in range(1,4):				
					...
					cmd = 'tar jcvf ' + domainName + '_pltl' + typeGoal + '_p0' + str(problemNumber) + '_hyp-' + str(hypNumber) + '_' + str(obs) + '_' + str(alternativeObs) + '.tar.bz2' + ' domain.pddl initial_state.pddl hyps.dat obs.dat observability.dat real_hyp.dat'
					os.system(cmd)
			...
			cmd = 'tar jcvf ' + domainName + '_pltl' + typeGoal + '_p0' + str(problemNumber) + '_hyp-' + str(hypNumber) + '_full.tar.bz2' + ' domain.pddl initial_state.pddl hyps.dat obs.dat observability.dat real_hyp.dat'
			os.system(cmd)
