...
class Action(object):
    def __init__(self, name, parameters, num_external_parameters,
                 precondition, effects, observation, cost):
    ...
    def parse(alist):
        ...
        if precondition_tag_opt == ":precondition":
            ...
            obs_tag_opt = next(iterator)
        else:
            precondition = conditions.Conjunction([])
            obs_tag_opt = precondition_tag_opt
        cost = None
        if obs_tag_opt == ":observe":
            observation_list = next(iterator) 
            obs = effects.parse_observation(observation_list)
            try:
                effect_tag_opt = next(iterator)
            except StopIteration:
                effect_tag_opt = ""
        else:
            obs = None
            effect_tag_opt = obs_tag_opt
        eff = []
        if effect_tag_opt == ":effect":
    ...
    def dump(self):
        ...
        print("Effects:")
        # FOND/POND
        for nondet_choice in self.effects:
            print("Nondeterministic choice:")
    ...
    def untyped(self):
        ...
        # FOND/POND
        result.effects = [[eff.untyped() for eff in nondet_choice] for nondet_choice in self.effects]
        return result

    # POND: handle unknown facts
    def instantiate(self, var_mapping, init_facts, fluent_facts, unknown_facts, objects_by_type):
        ...
        try:
            self.precondition.instantiate(var_mapping, init_facts,
                                          fluent_facts, unknown_facts, precondition)
        ...
        if self.observation is not None:
            for obs in self.observation:
                obs.instantiate(var_mapping, init_facts, fluent_facts, unknown_facts, observations)
            if len(observations) == 0:
                # only if this fact was static (and initially true or false)
                return None
            # if there is no effect, add it now
            if not self.effects:
                result.append(PropositionalAction(name, precondition, [], cost, observations))
        for nondet_choice in self.effects:
            effects = []
            for eff in nondet_choice:
                eff.instantiate(var_mapping, init_facts, fluent_facts, unknown_facts, 
                                objects_by_type, effects)
            result.append(PropositionalAction(name, precondition, effects, cost, observations))
        
        return result

# POND: added observations
class PropositionalAction:
    def __init__(self, name, precondition, effects, cost, observation):
        self.name = name
        self.precondition = precondition
        self.add_effects = []
        self.del_effects = []
