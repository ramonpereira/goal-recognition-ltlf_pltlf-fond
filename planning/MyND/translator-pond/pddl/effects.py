...
# POND
def parse_observation(alist):
    if isinstance(alist[0], list):
        return [conditions.Atom(obs[0], obs[1:]) for obs in alist]
    else:
        return [conditions.Atom(alist[0], alist[1:])]
...
class Effect(object):
    ...
    def instantiate(self, var_mapping, init_facts, fluent_facts, unknown_facts, 
        ...
        if self.parameters:
            ...
            for object_tuple in cartesian_product(*object_lists):
                ...
                self._instantiate(var_mapping, init_facts, fluent_facts, unknown_facts, result)
        else:
            self._instantiate(var_mapping, init_facts, fluent_facts, unknown_facts, result)
    def _instantiate(self, var_mapping, init_facts, fluent_facts, unknown_facts, result):
        condition = []
        try:
            self.condition.instantiate(var_mapping, init_facts, fluent_facts, unknown_facts, condition)
        except conditions.Impossible:
            return
        effects = []
        self.literal.instantiate(var_mapping, init_facts, fluent_facts, unknown_facts, effects)
