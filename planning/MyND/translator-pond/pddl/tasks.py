...
class Task(object):
    def __init__(self, domain_name, task_name, requirements, types, objects,
                 predicates, functions, init, init_unknown, init_oneof, 
                 init_formula, goal, actions, axioms, use_metric):
        ...
        self.init = init
        self.init_unknown = init_unknown
        self.init_oneof = init_oneof
    ...
    def parse(domain_pddl, task_pddl):
        ...
        task_name, task_domain_name, task_requirements, objects, init, init_unknown, init_oneof, init_formula, goal, use_metric = parse_task(task_pddl)
    ...
    def dump(self):
        ...
        for fact in self.init:
            print("  %s" % fact)
        print("Init Unknown:")
        for fact in self.init_unknown:
            print("  %s" % fact)
        print("Init OneOf:")
        for oneof in self.init_oneof:
            print ("  [",)
            for fact in oneof:
                print("%s" % fact)
            print("]")
        print("Init Formula:")
        print("  [",)
        for formula in self.init_formula:
            #print("%s" % formula)
            formula.dump()
        print("]")
        print("Goal:")
...
# TODO: POND - add requirement for POND
...
def parse_task(task_pddl):
    ...
    for fact in init[1:]:
        if fact[0] == "=":
            ...
            assert len(fact) == 2
            initial_unknown.append(conditions.Atom(fact[1][0], fact[1][1:]))
        elif fact[0] == "oneof":
            assert len(fact) >= 3
            list_of_oneofs = []
            for pair in fact[1:]:
                atom = conditions.Atom(pair[0], pair[1:])
                assert atom in initial_unknown
                list_of_oneofs.append(atom)
            initial_oneof.append(list_of_oneofs)
        elif fact[0] == "or":
            assert len(fact) >= 3
            initial_formula.append(conditions.parse_condition(fact))
        elif fact[0] == "and":
            assert len(fact) >= 3
            initial_formula.append(conditions.parse_condition(fact))
        elif fact[0] == "imply":
