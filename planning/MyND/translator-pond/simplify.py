...
from copy import deepcopy # POND
...
class DomainTransitionGraph(object):
    def __init__(self, init, size):
        self.init = init # POND: list of initial values
        self.size = size
    ...
    def reachable(self):
        queue = deepcopy(self.init) # TODO: is a copy is enough?
        reachable = set(queue)
        while queue:
            node = queue.pop()
...
def build_dtgs(task):
    # POND: for each variable which is unknown, we add all values as initial
    # value to the DTG.
    sizes = task.variables.ranges
    sorted_facts = sorted(task.init.facts) 
    var = 0
    dtgs = []
    while var < len(sizes):
        if (sorted_facts and sorted_facts[0][0] == var):
            val = sorted_facts[0][1]
            dtgs.append(DomainTransitionGraph([val], sizes[var]))
            sorted_facts = sorted_facts[1:]
        else:
            # value is unknown
            dtgs.append(DomainTransitionGraph(list(range(sizes[var])), sizes[var]))
        var = var + 1
...
class VarValueRenaming(object):
    ...
    def register_variable(self, old_domain_size, init_value, new_domain):
        ...
        for v in init_value:
            assert v in new_domain
        if len(new_domain) == 1:
            assert len(init_value) == 1 # len(new_domain) has to be >= len(init_value)
            # Remove this variable completely.
            #print("variable completely removed:", len(self.new_var_nos))
            new_values_for_var = [always_false] * old_domain_size
            new_values_for_var[init_value[0]] = always_true
    ...
    def apply_to_init(self, init):
        # POND facts
        init_pairs = init.facts
        try:
            self.translate_pairs_in_place(init_pairs)
        except Impossible:
            assert False, "Initial state impossible? Inconceivable!"
        # POND oneof facts
        init_oneof = init.facts_oneof
        new_init_oneof = []
        for oneof in init_oneof:
            new_oneof = []
            try:
                for pair in oneof:
                    new_oneof.append(self.translate_pair(pair))
                new_init_oneof.append(new_oneof)
            except Impossible:
                assert False, "Initial state impossible? Inconceivable!"
        init.facts_oneof = new_init_oneof
        # POND formulae
        init_formulae = init.formulae
        new_init_formulae = []
        for formula in init_formulae:
            new_formula = []
            for element in formula:
                if element in ["or(", "and(", ")"]:
                    new_formula.append(element)
                else:
                    try:
                        assert len(element) == 1
                        new_formula.append(self.translate_pair(element[0]))
                    except Impossible:
                        assert False, "Initial state impossible? Inconceivable!"
            new_init_formulae.append(new_formula)
        init.formulae = new_init_formulae
    ...
    def apply_to_operator(self, op):
        ...
        if new_pre_post == [[]] and not op.observation: 
            raise DoesNothing
        new_observation = []
        if op.observation: # POND
            try:
                for obs in op.observation:
                    try:
                        new_observation.append(self.translate_observation(obs))
                    except DoesNothing:
                        pass
                    op.observation = new_observation
                    if not new_observation:
                        # TODO: is it possible that there is an observation action
                        # with no observation, but with an effect?
                        assert new_pre_post == [[]]
                        raise DoesNothing # no observation and no effect
            except Impossible:
                assert False, "Observation impossible? Inconceivable!"
    ...
    def translate_observation(self, observation_tuple):
        (var_no, value) = observation_tuple
        new_var_no, new_value = self.translate_pair((var_no, value))
        if new_value is always_true:
            raise DoesNothing
